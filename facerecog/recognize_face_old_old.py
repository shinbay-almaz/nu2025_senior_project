"""
Face‑recognition demo with LCD + Edge‑TPU support.
Adds console logs, rate‑limits LCD updates, and now supports
arbitrary camera rotation via CORRECTION_ANGLE.

If you don’t have a Coral USB / PCIe, set `ifEdgeTPU_1_else_0` to 0.
"""
import re, time, math
...
CORRECTION_ANGLE = 25      # rotate frames (°). 0=no rotation. Positive = CCW.
...
def rotate_image(img: np.ndarray, angle: float) -> np.ndarray:
    """Rotate `img` by an arbitrary angle (° CCW) without cropping."""
    (h, w) = img.shape[:2]
    center = (w / 2, h / 2)
...
def apply_rotation(frame: np.ndarray) -> np.ndarray:
    """Rotate the frame according to CORRECTION_ANGLE."""
    ...
    # arbitrary angle
    return rotate_image(frame, angle)

# ───────────────────────── LCD helper ─────────────────────────────────────────
def lcd_show(line1: str = "", line2: str = "") -> None:
    """Update the LCD if content changed, respecting LCD_MIN_INTVAL."""
...
def main() -> None:
    ...
    try:
        fsm = FaceRecognizerFSM(cap, cv2, labels, in_h, in_w,
                                interpreter, interpreter_emb)
        fsm.enqueue("start_detection")
        fsm.run()
...
class FaceRecognizerFSM:
    def __init__(self, cap, cv2, labels, in_h, in_w, intr, intr_emb):
        self.cap, self.cv2, self.labels = cap, cv2, labels
        ...
        self.transitions = {
            ...
            ("recognizing","face_recognized"): ("checking"  , self.check_presence),
            ...
            ("remembering","successful")     : ("checking"  , self.check_presence),
    ...
    def run(self):
        while self.running and self.queue:
            ...
            if key not in self.transitions:
                print(f"[ERROR] Invalid transition: {key}")
                continue
    ...
    def detect_face(self):
        ...
        emb = detect_face_on_camera(self.cap, self.cv2, self.labels,
                                    self.in_h, self.in_w,
    ...
    def recognize_face(self, emb):
        ...
        if person == "Unknown":
            ...
            print(f"[INFO] recognised {person} (distance = {distance:.3f})")
            engine.say(f"Hello {person}"); engine.runAndWait()
            self.enqueue("face_recognized", person, emb)
    ...
    def remember_face(self, emb):
        ...
        self.enqueue("successful", name, emb)

    def check_presence(self, last_person, last_emb):
        lcd_show(last_person, "")
        curr_emb = detect_face_on_camera(self.cap, self.cv2, self.labels,
                                         self.in_h, self.in_w,
                                         self.interpreter, self.interpreter_emb)
        if curr_emb is None:
...
# ───────────────────── helper functions ───────────────────────────────────────
def calculate_similarity(e1, e2): return np.sum((e1 - e2) ** 2)
...
def detect_face_on_camera(cap, cv2, labels, in_h, in_w, intr, intr_emb):
    ok, frame = cap.read()
    if not ok: return None

#    frame = apply_rotation(frame)        # ← rotation applied here
